...
I created the market simulation model because I needed a fast and __painless__ way to obtain reasonable market values as an input into my big financial model.

The big financial model attempts to predict actual BMO P&L (e.g. revenues) and takes as an input various drivers, including the equity markets. The more accurate the input variables, the more meaningful the model predictions are.
...
My first attempt to model the returns is the use the popular normal distribution. To use the normal distribution, we only need two parameters:
