...
Visually, it doesn't appear that BMO revenues exhibit the highest volatility, but I concede that it's hard to definitely say based on visual inspection.
...
data.frame(BMO = sd(BMO$Rev.transform, na.rm = T) / mean(BMO$Rev.transform, na.rm = T) %>% round(4),
           RBC = sd(RBC$Rev.transform, na.rm = T) / mean(RBC$Rev.transform, na.rm = T) %>% round(4),
           NBC = sd(NBC$Rev.transform, na.rm = T) / mean(NBC$Rev.transform, na.rm = T) %>% round(4)
)
```

Voila. BMO revenue did in fact exhibit the lowest volatility compared to RBC and NBC.

Let's now return to Moody's analysis methods. Moody's proceeds to calculate the mean, standard deviation, and the coefficient of variation (sd / mean) of the three banks.

![](../../data/volatility/MoodyData.png)
