---
title: LSTM vs. Dense neural networks for time-series
author: Bruce Meng
date: '2018-06-17'
slug: lstm-vs-dense-neural-networks-for-time-series
categories: []
tags:
  - R
  - modelling
...
data.tibble.rec.lstm <- data.tibble.rec
...
```{r keras.lstm.train, message=F, include=FALSE}
# Train model
for (i in 1:5) {
        model.lstm %>% fit(x.lstm, y.lstm,
              batch_size = inputs,
              epochs = 1,
              shuffle = F,
              validation_split = 0.1,
...
```{r keras.show.lstm.model, message=F, echo = F}
# Model details
model.lstm
...
```{r keras.model.predict.lstm, message = F, warning = F}
## Predict based on last observed sunspot number
...
<!-- And for fun, let's see how a combined model looks like. -->
...
<!-- # Create model -->
<!-- model.comb <- keras_model_sequential() -->

<!-- model.comb %>% -->
<!--         layer_cudnn_lstm(units = units.lstm, -->
<!--                          input_shape = c(lookback, 1), -->
<!--                          batch_size = inputs, -->
<!--                          stateful = T, -->
<!--                          return_sequences = F -->
<!--                          ) %>% -->
<!--         layer_dense(units = units.dense, -->
<!--                     activation = "relu") %>% -->
<!--         # layer_dense(units = units.dense/2, -->
<!--         #             activation = "relu") %>% -->
<!--         # layer_dense(units = units.dense/8, -->
<!--         #             activation = "relu") %>% -->
<!--         layer_dense(units = 1) -->

<!-- # Compile model -->
<!-- model.comb %>% compile(optimizer = "rmsprop", -->
<!--                   loss = "mean_squared_error", -->
<!--                   metrics = "accuracy") -->
<!-- ``` -->
...
<!-- ```{r keras.model.predict, message = F, warning = F} -->
<!-- ## Predict based on last observed sunspot number -->
<!-- predictions.comb <- numeric() #vector to hold predictions.comb -->

<!-- # Generate predictions.comb, starting with last observed sunspot number and feeding -->
<!-- # new predictions.comb back into itself -->
<!-- for(i in 1:n){ -->
<!--     pred.y.comb <- x.comb[(nrow(x.comb) - inputs + 1):nrow(x.comb), 1:lookback, 1] -->
<!--     dim(pred.y.comb) <- c(inputs, lookback, 1) -->

<!--     # forecast -->
<!--     fcst.y.comb <- model.comb %>% predict(pred.y.comb, batch_size = inputs) -->
<!--     fcst.y.comb <- as_tibble(fcst.y.comb) -->
<!--     names(fcst.y.comb) <- "x" -->

<!--     # Add to previous dataset data.tibble.rec -->
<!--     data.tibble.rec.comb <- rbind(data.tibble.rec.comb, fcst.y.comb) -->

<!--     ## Recalc lag matrix.comb -->
<!--     # Setup a lagged matrix.comb (using helper function from nnfor) -->
<!--     data.tibble.rec.lag <- nnfor::lagmatrix(data.tibble.rec.comb$x, 0:lookback) -->
<!--     colnames(data.tibble.rec.lag) <- paste0("x-", 0:lookback) -->
<!--     data.tibble.rec.lag <- as_tibble(data.tibble.rec.lag) %>% -->
<!--     filter(!is.na(.[, ncol(.)])) %>% -->
<!--     as.matrix() -->

<!--     # x.comb is input (lag), y.comb is output, multiple inputs -->
<!--     x.comb <- data.tibble.rec.lag[, 2:(lookback + 1)] -->
<!--     dim(x.comb) <- c(nrow(x.comb), ncol(x.comb), 1) -->

<!--     y.comb <- data.tibble.rec.lag[, 1] -->
<!--     dim(y.comb) <- length(y.comb) -->

<!--     # Invert recipes -->
<!--     fcst.y.comb <- fcst.y.comb * (range.max.step - range.min.step) + range.min.step -->

<!--     # save prediction -->
<!--     predictions.comb[i] <- fcst.y.comb %>%  -->
<!--             InvBoxCox(l) -->
<!--     predictions.comb <- unlist(predictions.comb) -->
<!-- } -->
<!-- ``` -->
...
pred.lstm <- predictions.lstm %>%
        ts(start = date.end + 1/12, freq = 12)

pred.ensemble <- (pred.dense + pred.lstm)/2
...
g.plot <- ggplot(results, aes(x = Time, frame = Date)) +
        ...
        geom_line(aes(y = LSTM, linetype = "LSTM"), colour = "firebrick", show.legend = T) +
        geom_line(aes(y = Ensemble, linetype = "Ensemble"), colour = "green") +
...
The results are fairly good for all three models. I'll also calculate a numerical RMSE score to settle this one more definitively:

```{r rmse, echo = F}
rmse.dense <- Metrics::rmse(data.test, pred.dense)
rmse.lstm <- Metrics::rmse(data.test, pred.lstm)
rmse.ensemble <- Metrics::rmse(data.test, pred.ensemble)

rmse.results <- tibble(RMSE.Dense = rmse.dense, RMSE.LSTM = rmse.lstm, RMSE.Ensemble = rmse.ensemble)
