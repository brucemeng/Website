...
for(i in 1:n){
    ...
    # Invert recipes
    fcst.y <- (fcst.y + center.step + range.min.step) 
    fcst.y <- fcst.y * scale.step * (range.max.step - range.min.step)
...
g.plot <- ggplot(results, aes(x = Time, frame = Date)) +
        geom_line(aes(y = Actual, linetype = "Actual"), size = 1.1, alpha = 0.3) +
