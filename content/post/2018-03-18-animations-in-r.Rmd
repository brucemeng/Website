...
```{r gganimate, message = F, warning = F, fig.show="animate"}
library(gganimate)
library(tweenr)

# Plot
p <- ggplot(data.join, aes(x = income, y = babies)) +
        geom_point(aes(size = pop, frame = year, colour = region), alpha = 0.7) +  
        xlab("GDP per capita") +
        ylab("Number of Babies Born per Woman") +
        theme_minimal(base_size = 12, base_family = "Georgia") +
        geom_smooth(aes(group = year, frame = year), method = "loess", color = "black", linetype = "dashed", se = F, size = 0.5) +
...
data.join.tween <- left_join(data.join.tween, data.region.clean, by = c(".group" = "country"))
