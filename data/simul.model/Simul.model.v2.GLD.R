...
# V2 uses the Generalized Lambda Distribution

#setwd("/content/project/2017-10-22-market-simulation-model/")
...
simul.model <- function(init.value = 0, l = 0, num.month = 1, p = "fkml") {
    change <- rgl(num.month, lambda1 = l, param = p) #GLD or normal????
...
## TSX simulation
    # Obtain TSX data
    getSymbols("^GSPTSE", from ="2001-12-01")
    ...
    # Alternative Generalized Lambda Distribution fit
    gld.params.tsx <- fit.gpd(data.TSX.2010.change$change)

    # Replicate for MC simulation
    mc.TSX <- replicate(paths, simul.model(init.value = TSX.last.price$GSPTSE.Close,
                                         l = gld.params.tsx$estA,
                                         num.month = nbr.of.months,
                                         p = gld.params.tsx$param)) %>%
              data.frame()
...
## S&P500 Simulation
    ...
    # Alternative Generalized Lambda Distribution fit
    gld.params.SP500 <- fit.fkml(data.SP500.change$change)   

    # Replicate for MC simulation
    mc.SP500 <- replicate(paths, simul.model(init.value = SP500.last.price$GSPC.Close,
                                           l = gld.params.SP500$lambda,
                                           num.month = nbr.of.months,
                                           p = gld.params.SP500$param)) %>%
                data.frame()
...
## BoC Overnight rates
    ...
    boc.or <- Quandl("BOC/V39079", collapse = "monthly", type = "ts", start_date = "2001-12-01")/100
    
    boc.fit <- boc.or %>%
                    auto.arima(xreg = data.TSX$GSPTSE.Adjusted)
    
    boc.model <- forecast(boc.fit, h = 90, xreg = mc.TSX$Avg[-1])
    boc.model.df <- data.frame(Date = index(boc.model$mean), OR = boc.model$mean)

## Write data
    #write_csv(mc.TSX, "../../../data/simul.model/Output/mc.TSX.csv")
    #write_csv(mc.SP500, "../../../data/simul.model/Output/mc.SP500.csv")
    #write_csv(boc.model.df, "../../../data/simul.model/Output/boc.model.csv")
